...
function decodeClaudePath(encodedName) {
  // D--projects-voice-app -> D:\projects\voice_app
  // Handle drive letter: first part before -- is drive letter
  const parts = encodedName.split('-');

  if (parts.length === 0) return encodedName;

  // Find the drive letter (single char followed by empty string from --)
  let result = '';
  let i = 0;
...
async function scanForContextFolders() {
  ...
  try {
    ...
    const projectFolders = fs.readdirSync(claudeProjectsDir, { withFileTypes: true });
